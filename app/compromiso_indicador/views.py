...
from .queries import (obtener_avance_compromiso_indicador, obtener_variables_compromiso_indicador, obtener_avance_regional_mensual_compromiso_indicador,
                    obtener_cobertura_por_zona, obtener_cobertura_por_provincia, obtener_cobertura_por_distrito, 
                    obtener_seguimiento_compromiso_indicador,
                    obtener_avance_regional_mensual_compromiso_indicador, obtener_avance_cobertura_compromiso_indicador, 
                    obtener_cobertura_por_red, obtener_cobertura_por_microred, obtener_cobertura_por_establecimiento, 
                    obtener_seguimiento_compromiso_indicador_red, obtener_seguimiento_compromiso_indicador_microred, obtener_seguimiento_compromiso_indicador_establecimiento)
...
def BASE(request):
    ...
    return render(request,'compromiso_indicador/index_compromiso_indicador.html', {"actualizacion": actualizacion})
############################
...
def process_avance_regional_mensual_compromiso_indicador(resultados_avance_regional_mensual_compromiso_indicador):
    ...
    for index, row in enumerate(resultados_avance_regional_mensual_compromiso_indicador):
        try:
...
def process_variables_por_region(resultados_variables_por_region):
    ...
    if not resultados_variables_por_region:
        data['i_den_variable'] = [0]
        data['i_num_sup_ta'] = [0]
        data['i_avance_sup_ta'] = [0.0]
        data['i_num_hb'] = [0]
        data['i_avance_hb'] = [0.0]
        data['i_num_hb_12m'] = [0]
        data['i_avance_hb_12m'] = [0.0]
        data['i_num_sesion'] = [0]
        data['i_avance_sesion'] = [0.0]
    ...
    try:
        ...
        num_sup_ta = row.get('num_sup_ta', 0)
        avance_sup_ta = row.get('avance_sup_ta', 0.0)
        num_hb = row.get('num_hb', 0)
        avance_hb = row.get('avance_hb', 0.0)
        num_hb_12m = row.get('num_hb_12m', 0)
        avance_hb_12m = row.get('avance_hb_12m', 0.0)
        num_sesion = row.get('num_sesion', 0)     
        avance_sesion = row.get('avance_sesion', 0.0)
        

        # Asegurar que los valores no sean None        
        den_variable = den_variable if den_variable is not None else 0
        num_sup_ta = num_sup_ta if num_sup_ta is not None else 0
        avance_sup_ta = avance_sup_ta if avance_sup_ta is not None else 0.0
        num_hb = num_hb if num_hb is not None else 0
        avance_hb = avance_hb if avance_hb is not None else 0.0
        num_hb_12m = num_hb_12m if num_hb_12m is not None else 0
        avance_hb_12m = avance_hb_12m if avance_hb_12m is not None else 0.0
        num_sesion = num_sesion if num_sesion is not None else 0
        avance_sesion = avance_sesion if avance_sesion is not None else 0.0
        

        data['i_den_variable'].append(int(den_variable))
        data['i_num_sup_ta'].append(int(num_sup_ta))
        data['i_avance_sup_ta'].append(float(avance_sup_ta))
        data['i_num_hb'].append(int(num_hb))
        data['i_avance_hb'].append(float(avance_hb))
        data['i_num_hb_12m'].append(int(num_hb_12m))
        data['i_avance_hb_12m'].append(float(avance_hb_12m))
        data['i_num_sesion'].append(int(num_sesion))
        data['i_avance_sesion'].append(float(avance_sesion))
        ...
        data['i_den_variable'] = [0]
        data['i_num_sup_ta'] = [0]
        data['i_avance_sup_ta'] = [0.0] # Corrected from v_avani_avance_sup_tace_cred
        data['i_num_hb'] = [0]
        data['i_avance_hb'] = [0.0]
        data['i_num_hb_12m'] = [0]
        data['i_avance_hb_12m'] = [0.0]
...
##----------------------------
def process_avance_cobertura_compromiso_indicador(resultados_avance_cobertura_compromiso_indicador):
    """Procesa los resultados del graficos"""
    data = {
            'anio': [],
            'mes': [],
    ...
    for row in resultados_avance_cobertura_compromiso_indicador:
        try:
...
## PANTALLA PRINCIPAL
#######################
def index_compromiso_indicador(request):
    actualizacion = Actualizacion.objects.all()
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        try:
            ...
            resultados_avance_por_region = obtener_avance_compromiso_indicador(anio, mes_seleccionado_inicio, mes_seleccionado_fin, provincia_seleccionada, distrito_seleccionado)
            resultados_variables_por_region = obtener_variables_compromiso_indicador(anio, mes_seleccionado_inicio, mes_seleccionado_fin, provincia_seleccionada, distrito_seleccionado)           
            resultados_avance_regional_mensual_compromiso_indicador = obtener_avance_regional_mensual_compromiso_indicador(anio, mes_seleccionado_inicio, mes_seleccionado_fin, provincia_seleccionada, distrito_seleccionado) 
            ...
            resultados_avance_cobertura_compromiso_indicador = obtener_avance_cobertura_compromiso_indicador(anio, mes, red_seleccionada, microred_seleccionada, establecimiento_seleccionado, provincia, distrito)
            ...
            data = {
                ...
                **process_avance_regional_mensual_compromiso_indicador(resultados_avance_regional_mensual_compromiso_indicador),
                **process_avance_cobertura_compromiso_indicador(resultados_avance_cobertura_compromiso_indicador),
    ...
    return render(request, 'compromiso_indicador/index_compromiso_indicador.html', {
...
##----------------------------------
def get_establecimientos_compromiso_indicador_h(request,establecimiento_id):
    redes_h = (
                MAESTRO_HIS_ESTABLECIMIENTO
                .objects.filter(Descripcion_Sector='GOBIERNO REGIONAL',Disa='JUNIN')
                .annotate(codigo_red_filtrado=Substr('Codigo_Red', 1, 4))
    ...
    return render(request,'compromiso_indicador/establecimientos_h.html', context)

def p_microredes_establec_compromiso_indicador_h(request):
    ...
    return render(request, 'compromiso_indicador/partials/p_microredes_establec_h.html', context)

def p_establecimientos_compromiso_indicador_h(request):
    ...
    return render(request, 'compromiso_indicador/partials/p_establecimientos_h.html', context)

##-----------------------------------
## FILTROS HORIZONTAL POR MUNICIPIO
##-----------------------------------
def p_distritos_compromiso_indicador_h(request):
    provincias_h = (
                MAESTRO_HIS_ESTABLECIMIENTO
                .objects.filter(Descripcion_Sector='GOBIERNO REGIONAL')
                .annotate(ubigueo_filtrado=Substr('Ubigueo_Establecimiento', 1, 4))
    ...
    return render(request, 'compromiso_indicador/partials/p_distritos.html', context)
...
def get_redes_compromiso_indicador(request,redes_id):
    redes = (
            MAESTRO_HIS_ESTABLECIMIENTO
    ...
    return render(request, 'compromiso_indicador/components/salud/redes.html', context)

## SEGUIMIENTO POR MICRO-REDES
def get_microredes_compromiso_indicador(request, microredes_id):
    redes = (
            MAESTRO_HIS_ESTABLECIMIENTO
            .objects.filter(Descripcion_Sector='GOBIERNO REGIONAL',Departamento='JUNIN')
            .annotate(codigo_red_filtrado=Substr('Codigo_Red', 1, 4))
    ...
    return render(request, 'compromiso_indicador/components/salud/microredes.html', context)

def p_microredes_compromiso_indicador(request):
    ...
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        return render(request, 'compromiso_indicador/partials/p_microredes_options.html', context)
    
    return render(request, 'compromiso_indicador/partials/p_microredes.html', context)

## REPORTE POR ESTABLECIMIENTO
def get_establecimientos_compromiso_indicador(request,establecimiento_id):
    redes = (
                MAESTRO_HIS_ESTABLECIMIENTO
                .objects.filter(Descripcion_Sector='GOBIERNO REGIONAL',Disa='JUNIN')
                .annotate(codigo_red_filtrado=Substr('Codigo_Red', 1, 4))
    ...
    return render(request,'compromiso_indicador/components/salud/establecimientos.html', context)

def p_microredes_establec_compromiso_indicador(request):
    redes_param = request.GET.get('red') 
    ...
    return render(request, 'compromiso_indicador/partials/p_microredes_establec.html', context)

def p_establecimientos_compromiso_indicador(request):
    ...
    # Si es una petición AJAX, devolver solo las opciones
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        return render(request, 'compromiso_indicador/partials/p_establecimientos_options.html', context)
    
    return render(request, 'compromiso_indicador/partials/p_establecimientos.html', context)
...
def get_provincias_compromiso_indicador(request, provincia_id):
    provincias = (
                MAESTRO_HIS_ESTABLECIMIENTO
                .objects.filter(Descripcion_Sector='GOBIERNO REGIONAL')
                .annotate(ubigueo_filtrado=Substr('Ubigueo_Establecimiento', 1, 4))
    ...
    return render(request, 'compromiso_indicador/components/municipio/provincias.html', context)

## SEGUIMIENTO POR DISTRITOS
def get_distritos_compromiso_indicador(request, distrito_id):
    provincias = (
                MAESTRO_HIS_ESTABLECIMIENTO
                .objects.filter(Descripcion_Sector='GOBIERNO REGIONAL')
                .annotate(ubigueo_filtrado=Substr('Ubigueo_Establecimiento', 1, 4))
    ...
    return render(request, 'compromiso_indicador/components/municipio/distritos.html', context)

def p_distrito_compromiso_indicador(request):
...
## REPORTE DE EXCEL
class RptIndicadorCompromiso(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        resultado_seguimiento_compromiso_indicador = obtener_seguimiento_compromiso_indicador(anio, mes_inicio, mes_fin, provincia, distrito, p_red, p_microredes, p_establecimiento, p_cumple)
        
        wb = Workbook()
        
        consultas = [
                ('Seguimiento', resultado_seguimiento_compromiso_indicador)
        ]
        ...
        nombre_archivo = "rpt_compromiso_indicador.xlsx"
...
class RptPnPoblacionMicroRed(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        p_cumple = request.GET.get('cumple', '') 
        # Creación de la consulta
        resultado_seguimiento_microred = obtener_seguimiento_compromiso_indicador_microred(p_departamento, p_red, p_microred, p_edades, p_cumple)
        ...
        ##########################################################################          
        # Establecer el nombre del archivo
        nombre_archivo = "rpt_compromiso_indicador_microred.xlsx"
        # Definir el tipo de respuesta que se va a dar
        response = HttpResponse(content_type="application/ms-excel")
        contenido = "attachment; filename={}".format(nombre_archivo)
        response["Content-Disposition"] = contenido
...
class RptPnPoblacionEstablec(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        # Creación de la consulta
        resultado_seguimiento = obtener_seguimiento_compromiso_indicador_establecimiento(p_departamento,p_establec,p_edades,p_cumple)
        ...
        ##########################################################################          
        # Establecer el nombre del archivo
        nombre_archivo = "rpt_compromiso_indicador_establecimiento.xlsx"
        # Definir el tipo de respuesta que se va a dar
        response = HttpResponse(content_type="application/ms-excel")
        contenido = "attachment; filename={}".format(nombre_archivo)
        response["Content-Disposition"] = contenido
