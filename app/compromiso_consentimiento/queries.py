...
def obtener_avance_compromiso_consentimiento(anio, mes_inicio, mes_fin, provincia, distrito):
    try:
        with connection.cursor() as cursor:
            ...
            sql_query = '''
                SELECT 
                    ...
                    END AS avance
                FROM 
                    Compromiso_1.dbo.indicador_compromiso_consentimiento
            '''
            params = []
            conditions = []
            ...
            # Filtros de ubicación geográfica - usando LIKE para códigos de ubigueo
            if provincia and provincia != '':
                conditions.append("LEFT(ubigueo, 4) = %s")
                params.append(provincia)
                #print(f"Filtro provincia aplicado: LEFT(ubigueo, 4) = {provincia}")
            
            if distrito and distrito != '':
                conditions.append("ubigueo = %s")
                params.append(distrito)
                #print(f"Filtro distrito aplicado: ubigueo = {distrito}")
...
def obtener_resumen_compromiso_consentimiento(anio, mes_inicio, mes_fin, provincia, distrito):
    """
    Obtiene un resumen detallado del paquete compromiso con información adicional
    """
    datos_base = obtener_avance_compromiso_consentimiento(anio, mes_inicio, mes_fin, provincia, distrito)
...
def obtener_avance_regional_mensual_compromiso_consentimiento(anio, mes_inicio, mes_fin, provincia, distrito):
    try:
        with connection.cursor() as cursor:
            sql_query = '''
                    SELECT
                        ...
                        CASE
                            ...
                            , 2)
                        END AS cob_12
                    FROM Compromiso_1.dbo.indicador_compromiso_consentimiento
            '''
            params = []
            conditions = []
...
def obtener_variables_compromiso_consentimiento(anio, mes_inicio, mes_fin, provincia, distrito):
    try:
        with connection.cursor() as cursor:
            ...
            sql_query = '''
                SELECT 
                    ...
                    SUM(ISNULL(CAST(num_apm AS INT), 0)) AS num_apn,
                        CASE 
                        WHEN SUM(ISNULL(CAST(denominador AS INT), 0)) = 0 THEN 0.0
                        ELSE ROUND(
                            (SUM(ISNULL(CAST(num_apm AS INT), 0)) * 100.0) / 
                            SUM(ISNULL(CAST(denominador AS INT), 0)), 2
                        )
                    END AS avance_apn,
                    -- 1° consentimiento
                    SUM(ISNULL(CAST(num_consentimiento AS INT), 0)) AS num_consentimiento,
                    CASE 
                        WHEN SUM(ISNULL(CAST(denominador AS INT), 0)) = 0 THEN 0.0
                        ELSE ROUND(
                           (SUM(ISNULL(CAST(num_consentimiento AS INT), 0)) * 100.0) / 
                            SUM(ISNULL(CAST(denominador AS INT), 0)), 2
                        )
                    END AS avance_consentimiento
                FROM 
                    Compromiso_1.dbo.indicador_compromiso_consentimiento
            '''
            params = []
...
def obtener_ranking_compromiso_consentimiento(anio, mes, red, microred, establecimiento, provincia, distrito):
    with connection.cursor() as cursor:
        ...
        sql_query = """
            ...
            FROM 
                consentimientoes_FED.dbo.MC01_PaqueteGestante_Combinado
            WHERE 1=1
        """
        params = []
...
## AVANCE REGIONAL
def obtener_avance_regional_compromiso_consentimiento():
    try:
...
def obtener_avance_cobertura_compromiso_consentimiento(anio, mes, red_h, p_microredes_establec_h, p_establecimiento_h, provincia, distrito):
    """
...
def obtener_seguimiento_compromiso_consentimiento(anio, mes_inicio, mes_fin, provincia, distrito, p_red, p_microredes, p_establecimiento, p_cumple):
    try:
        with connection.cursor() as cursor:
            ...
            # Filtros de salud - usando LIKE para códigos de ubigueo    
            if p_red and p_red != '':
            ...
            # Agregar filtro de cumplimiento del consentimiento
            if p_cumple == '1':
...
### --- 
def obtener_seguimiento_compromiso_consentimiento_red(departamento, red, edad, cumple):
    """
    ...
    with connection.cursor() as cursor:
        ...
        query = f'''
                SELECT 
                    ...
                    REFERENCIA_DIRECCION, COD_ubigueo, DEPARTAMENTO, PROVINCIA, DISTRITO, CODIGO_CENTRO_POBLADO, 
...
def obtener_seguimiento_compromiso_consentimiento_microred(departamento, red, microred, edad, cumple):
    """
    Función para obtener datos del seguimiento del padrón nominal filtrados por ubicación, edad y cumplimiento.
...
def obtener_seguimiento_compromiso_consentimiento_establecimiento(departamento, establecimiento, edad, cumple):
    """
    Función para obtener datos del seguimiento del padrón nominal filtrados por ubicación, edad y cumplimiento.
