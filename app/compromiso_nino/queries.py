        ...
        cursor.execute(
            '''
                SELECT 
                    ...
                    CAST(
                        CASE 
                            WHEN SUM(den) = 0 THEN 0
                            ELSE ROUND(SUM(num) * 100.0 / SUM(den), 2)
                        END AS DECIMAL(10,2)
                    ) AS avance_ranking,
                    CASE 
                        WHEN SUM(den) = 0 THEN 'RIESGO'
                        WHEN ROUND(SUM(num) * 100.0 / SUM(den), 2) >= 90 THEN 'CUMPLE'
                        WHEN ROUND(SUM(num) * 100.0 / SUM(den), 2) >= 80 THEN 'PROCESO'
                        ELSE 'RIESGO'
...
def obtener_seguimiento_compromiso(departamento, provincia, edad, cumple):
    """
...
def obtener_seguimiento_comprimiso_distrito(departamento, provincia, distrito, edad, cumple):
    # Mapeo de las categorías de edad a condiciones SQL
    edad_conditions = {
        "": "1=1",  # Todos los registros si no se selecciona ninguna edad
        "N28_dias": "(edad_anio2 = 0 AND edad_mes2 = 0 AND edad_dias2 < 28)",  
