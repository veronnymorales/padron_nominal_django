...
from .queries import (obtener_distritos, obtener_ranking_zona, obtener_ranking_distritos, obtener_seguimiento_compromiso, obtener_seguimiento_comprimiso_distrito)
...
def index_compromiso_nino(request):
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        ...
        try:
            ...
            resultados_ranking_zona = obtener_ranking_zona()
            print("resultados_ranking_zona:", resultados_ranking_zona)
            ...
            # ----------------------------------------------------------------------------
            for row in resultados_ranking_zona:
    ...
    return render(request, 'compromiso_nino/index_compromiso_nino.html', {
...
def get_provincias_compromiso(request, provincia_id):
    provincias = (
...
def get_distritos_compromiso(request, distrito_id):
    provincias = (
                MAESTRO_HIS_ESTABLECIMIENTO
                .objects.filter(Descripcion_Sector='GOBIERNO REGIONAL')
                .annotate(ubigueo_filtrado=Substr('Ubigueo_Establecimiento', 1, 4))
...
def p_distrito_compromiso(request):
...
class RptSituacionProvincia(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        resultado_seguimiento = obtener_seguimiento_compromiso(p_departamento, p_provincia, p_edades, p_cumple)
...
class RptSituacionDistrito(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        # Creación de la consulta
        resultado_seguimiento = obtener_seguimiento_comprimiso_distrito(p_departamento, p_provincia, p_distrito, p_edades, p_cumple)
        ...
        ##########################################################################          
        # Establecer el nombre del archivo
        nombre_archivo = "rpt_compromiso_distrito.xlsx"
        # Definir el tipo de respuesta que se va a dar
        response = HttpResponse(content_type="application/ms-excel")
        contenido = "attachment; filename={}".format(nombre_archivo)
        response["Content-Disposition"] = contenido
