...
from .queries import (obtener_distritos, obtener_avance_pn_poblacion, obtener_ranking_pn_poblacion, obtener_avance_regional_pn_poblacion, 
                    obtener_avance_regional_mensual_pn_poblacion, obtener_avance_cobertura_pn_poblacion, obtener_cobertura_por_edad, 
                    obtener_cobertura_por_red, obtener_cobertura_por_microred, obtener_cobertura_por_establecimiento, 
                    obtener_seguimiento_pn_poblacion_red, obtener_seguimiento_pn_poblacion_microred, obtener_seguimiento_pn_poblacion_establecimiento)
...
def BASE(request):
    ...
    return render(request,'pn_poblacion/index_pn_poblacion.html', {"actualizacion": actualizacion})
############################
## COMPONENTES Y GRAFICOS 
############################
def process_avance_regional_mensual_pn_poblacion(resultados_avance_regional_mensual_pn_poblacion):
    ...
    for index, row in enumerate(resultados_avance_regional_mensual_pn_poblacion):
        try:
...
def process_avance_cobertura_pn_poblacion(resultados_avance_cobertura_pn_poblacion):
    """Procesa los resultados del graficos"""
    data = {
            'anio': [],
            'mes': [],
    ...
    for row in resultados_avance_cobertura_pn_poblacion:
        try:
...
## PANTALLA PRINCIPAL
#######################
def index_pn_poblacion(request):
    actualizacion = Actualizacion.objects.all()

    # Capturamos el año que viene por GET
    anio = request.GET.get('anio', '2025')
    mes = request.GET.get('mes', '')
    red = request.GET.get('red_h', '')
    microred = request.GET.get('p_microredes_establec_h', '')
    establecimiento = request.GET.get('p_establecimiento_h', '')
    ...
    print(f"Red: {red_seleccionada}, Microred: {microred_seleccionada}, Establecimiento: {establecimiento_seleccionado}")  # Imprime los parámetros
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        try:
            resultados_avance_regional_mensual_pn_poblacion = obtener_avance_regional_mensual_pn_poblacion(anio, mes, red_seleccionada, microred_seleccionada, establecimiento_seleccionado)
            resultados_avance_cobertura_pn_poblacion = obtener_avance_cobertura_pn_poblacion(anio, mes, red_seleccionada, microred_seleccionada, establecimiento_seleccionado, provincia, distrito)
            ...
            #print("resultados_cobertura_por_microred:", resultados_cobertura_por_microred)
            
            # Check for empty results and handle them
            if not resultados_avance_regional_mensual_pn_poblacion:
            ...
            data = {
                **process_avance_regional_mensual_pn_poblacion(resultados_avance_regional_mensual_pn_poblacion),
                **process_avance_cobertura_pn_poblacion(resultados_avance_cobertura_pn_poblacion),
    ...
    return render(request, 'pn_poblacion/index_pn_poblacion.html', {
...
##----------------------------------
def get_establecimientos_pn_poblacion_h(request,establecimiento_id):
    redes_h = (
                MAESTRO_HIS_ESTABLECIMIENTO
                .objects.filter(Descripcion_Sector='GOBIERNO REGIONAL',Disa='JUNIN')
                .annotate(codigo_red_filtrado=Substr('Codigo_Red', 1, 4))
    ...
    return render(request,'pn_poblacion/establecimientos_h.html', context)

def p_microredes_establec_pn_poblacion_h(request):
    ...
    return render(request, 'pn_poblacion/partials/p_microredes_establec_h.html', context)

def p_establecimientos_pn_poblacion_h(request):
    ...
    return render(request, 'pn_poblacion/partials/p_establecimientos_h.html', context)

##-----------------------------------
## FILTROS HORIZONTAL POR MUNICIPIO
##-----------------------------------
def p_distritos_pn_poblacion_h(request):
    provincias_h = (
                MAESTRO_HIS_ESTABLECIMIENTO
                .objects.filter(Descripcion_Sector='GOBIERNO REGIONAL')
                .annotate(ubigueo_filtrado=Substr('Ubigueo_Establecimiento', 1, 4))
    ...
    return render(request, 'pn_poblacion/partials/p_distritos.html', context)
...
def get_redes_pn_poblacion(request,redes_id):
    redes = (
            MAESTRO_HIS_ESTABLECIMIENTO
    ...
    return render(request, 'pn_poblacion/components/salud/redes.html', context)

## SEGUIMIENTO POR MICRO-REDES
def get_microredes_pn_poblacion(request, microredes_id):
    redes = (
            MAESTRO_HIS_ESTABLECIMIENTO
            .objects.filter(Descripcion_Sector='GOBIERNO REGIONAL',Departamento='JUNIN')
            .annotate(codigo_red_filtrado=Substr('Codigo_Red', 1, 4))
    ...
    return render(request, 'pn_poblacion/components/salud/microredes.html', context)

def p_microredes_pn_poblacion(request):
    ...
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        return render(request, 'pn_poblacion/partials/p_microredes_options.html', context)
    
    return render(request, 'pn_poblacion/partials/p_microredes.html', context)

## REPORTE POR ESTABLECIMIENTO
def get_establecimientos_pn_poblacion(request,establecimiento_id):
    redes = (
                MAESTRO_HIS_ESTABLECIMIENTO
                .objects.filter(Descripcion_Sector='GOBIERNO REGIONAL',Disa='JUNIN')
                .annotate(codigo_red_filtrado=Substr('Codigo_Red', 1, 4))
    ...
    return render(request,'pn_poblacion/components/salud/establecimientos.html', context)

def p_microredes_establec_pn_poblacion(request):
    redes_param = request.GET.get('red') 
    ...
    return render(request, 'pn_poblacion/partials/p_microredes_establec.html', context)

def p_establecimientos_pn_poblacion(request):
    ...
    # Si es una petición AJAX, devolver solo las opciones
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        return render(request, 'pn_poblacion/partials/p_establecimientos_options.html', context)
    
    return render(request, 'pn_poblacion/partials/p_establecimientos.html', context)
...
def get_provincias_pn_poblacion(request, provincia_id):
    provincias = (
                MAESTRO_HIS_ESTABLECIMIENTO
                .objects.filter(Descripcion_Sector='GOBIERNO REGIONAL')
                .annotate(ubigueo_filtrado=Substr('Ubigueo_Establecimiento', 1, 4))
    ...
    return render(request, 'pn_poblacion/components/municipio/provincias.html', context)

## SEGUIMIENTO POR DISTRITOS
def get_distritos_pn_poblacion(request, distrito_id):
    provincias = (
                MAESTRO_HIS_ESTABLECIMIENTO
                .objects.filter(Descripcion_Sector='GOBIERNO REGIONAL')
                .annotate(ubigueo_filtrado=Substr('Ubigueo_Establecimiento', 1, 4))
    ...
    return render(request, 'pn_poblacion/components/municipio/distritos.html', context)

def p_distrito_pn_poblacion(request):
    provincia_param = request.GET.get('provincia', '')
...
class RptPnPoblacionRed(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        resultado_seguimiento = obtener_seguimiento_pn_poblacion_red(p_departamento, p_red, p_edades, p_cumple)
        ...
        for index, (sheet_name, results) in enumerate(consultas):
            ...
            fill_worksheet_pn_poblacion(ws, results)
        
        ##########################################################################          
        # Establecer el nombre del archivo
        nombre_archivo = "rpt_pn_poblacion_red.xlsx"
...
class RptPnPoblacionMicroRed(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        p_cumple = request.GET.get('cumple', '') 
        # Creación de la consulta
        resultado_seguimiento_microred = obtener_seguimiento_pn_poblacion_microred(p_departamento, p_red, p_microred, p_edades, p_cumple)
        ...
        ##########################################################################          
        # Establecer el nombre del archivo
        nombre_archivo = "rpt_pn_poblacion_microred.xlsx"
        # Definir el tipo de respuesta que se va a dar
        response = HttpResponse(content_type="application/ms-excel")
        contenido = "attachment; filename={}".format(nombre_archivo)
        response["Content-Disposition"] = contenido
...
class RptPnPoblacionEstablec(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        # Creación de la consulta
        resultado_seguimiento = obtener_seguimiento_pn_poblacion_establecimiento(p_departamento,p_establec,p_edades,p_cumple)
        ...
        ##########################################################################          
        # Establecer el nombre del archivo
        nombre_archivo = "rpt_pn_poblacion_establecimiento.xlsx"
        # Definir el tipo de respuesta que se va a dar
        response = HttpResponse(content_type="application/ms-excel")
        contenido = "attachment; filename={}".format(nombre_archivo)
        response["Content-Disposition"] = contenido
...
def fill_worksheet_pn_poblacion(ws, results): 
